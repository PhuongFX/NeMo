...
def test_set_model_parallel_attributes() -> None:
    ...
    class DummyModel:
       def __init__(self):
           self.config = TransformerConfig(hidden_size=128, num_attention_heads=2)
    model = DummyModel()
    assert model.pipeline_model_parallel_size != 2
    assert model.expert_model_parallel_size != 2
    assert model.sequence_parallel != False
    assert model.pipeline_dtype != torch.float32
